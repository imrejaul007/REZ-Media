...
router.post('/send', async (req: Request, res: Response) => {
  ...
  if (segment === 'all') {
    ...
    userIds = docs.map((d: any) => String(d));
    ...
    userIds = result.map((r: any) => String(r._id));
...
router.post('/', async (req: Request, res: Response) => {
  ...
  if (createdCampaigns.length === 0) {
    return res.status(500).json(err('SRV_001', 'Failed to create any broadcast campaigns'));
  }
...
router.get('/:merchantId', async (req: Request, res: Response) => {
  ...
  const broadcasts = campaigns.map(c => ({
    broadcastId: (c._id as any).toString(),
    name: c.name,
    channel: c.channel,
