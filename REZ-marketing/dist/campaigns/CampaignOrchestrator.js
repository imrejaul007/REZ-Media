...
exports.campaignOrchestrator = exports.CampaignOrchestrator = void 0;
...
const WhatsAppChannel_1 = require("../channels/WhatsAppChannel");
 ...
 */
const dispatchQueue = new bullmq_1.Queue('mkt-campaigns', { connection: (0, redis_1.getRedisBullMQConnection)() });
class CampaignOrchestrator {
    ...
    async dispatch(campaignId) {
        ...
        if (!locked)
            throw new Error('Campaign dispatch already in progress');
        ...
        logger_1.logger.info('[Orchestrator] Campaign enqueued', { campaignId });
    ...
    async execute(campaignId) {
        ...
        if (campaign.status !== 'scheduled' && campaign.status !== 'sending') {
            logger_1.logger.warn('[Orchestrator] Campaign not in executable state — skipping', {
                campaignId,
        ...
        // Mark sending
        await MarketingCampaign_1.MarketingCampaign.findByIdAndUpdate(campaignId, { status: 'sending' });
        ...
        const recheckTimeout = new Promise((_, reject) => setTimeout(() => reject(new Error('Campaign status check timed out')), 5000));
        const recheck = await Promise.race([
            MarketingCampaign_1.MarketingCampaign.findById(campaignId).select('status').lean(),
            recheckTimeout,
        ]).catch((err) => {
            logger_1.logger.warn('[Orchestrator] Campaign status check timed out — proceeding optimistically', {
                campaignId,
                error: err.message,
            });
            return null;
        ...
        if (!recheck || recheck.status !== 'sending') {
            logger_1.logger.info('[Orchestrator] Campaign no longer in sending state — skipping', {
                campaignId,
        ...
        let audienceGen;
        try {
            audienceGen = AudienceBuilder_1.audienceBuilder.buildAudience(campaign.merchantId.toString(), campaign.audience, campaign.channel);
        }
        catch (err) {
            logger_1.logger.error('[Orchestrator] Audience generation failed', { campaignId, error: err.message });
        ...
        try {
            for await (const batch of audienceGen) {
                ...
                const midCheck = await MarketingCampaign_1.MarketingCampaign.findById(campaignId).select('status dailyBudget totalSpent').lean();
                if (!midCheck || midCheck.status === 'cancelled') {
                    logger_1.logger.info('[Orchestrator] Campaign cancelled mid-execution', { campaignId });
                    finalStatus = 'failed';
                    break;
                }
                // P0-LOGIC-3: Check budget before dispatching this batch
                if (midCheck.dailyBudget && (midCheck.totalSpent || 0) >= midCheck.dailyBudget) {
                    logger_1.logger.info('[Orchestrator] Daily budget exhausted, stopping campaign', {
                        campaignId,
                ...
                for (const customer of batch) {
                    ...
                    const personalizedMsg = finalMessage.replace(/\{\{name\}\}/g, extractFirstName(customer) || 'valued customer');
                    let result;
                    switch (campaign.channel) {
                        ...
                        case 'sms': {
                            ...
                            result = await SMSChannel_1.smsChannel.send({
                                to: customer.phone,
                        ...
                        case 'push': {
                            ...
                            const pushResult = await PushChannel_1.pushChannel.send({
                                tokens,
                                title: campaign.name,
                        ...
                        case 'email': {
                            ...
                            const html = EmailChannel_1.emailChannel.buildHtml(personalizedMsg, campaignId, campaign.ctaUrl, campaign.ctaText);
                            result = await EmailChannel_1.emailChannel.send({
                                to: customer.email,
                                subject,
                                html,
                                campaignId,
                    ...
                    if (result.deduped)
                        ...
                        if (midCheck.dailyBudget) {
                            await MarketingCampaign_1.MarketingCampaign.findOneAndUpdate({
                                _id: campaignId,
                ...
                if (campaign.channel === 'whatsapp' && waBatch.length > 0) {
                    ...
                    const batchResult = await WhatsAppChannel_1.whatsAppChannel.sendBatch(waOptions);
            ...
            logger_1.logger.error('[Orchestrator] Campaign dispatch loop failed', { campaignId, error: err.message });
            finalStatus = 'failed';
        }
        ...
        await MarketingCampaign_1.MarketingCampaign.findByIdAndUpdate(campaignId, {
            ...
            sentAt: new Date(),
            stats,
            ...(campaignFinalStatus === 'failed' && !(await MarketingCampaign_1.MarketingCampaign.findById(campaignId).select('errorMessage').lean())?.errorMessage
                ? { errorMessage: 'All dispatch attempts failed' }
                : {}),
        });
        logger_1.logger.info('[Orchestrator] Campaign complete', { campaignId, finalStatus: campaignFinalStatus, stats });
        return stats;
    }
}
...
exports.campaignOrchestrator = new CampaignOrchestrator();
exports.default = exports.campaignOrchestrator;
//# sourceMappingURL=CampaignOrchestrator.js.map