...
exports.clickFraudService = void 0;
// @ts-nocheck
const redis_1 = require("../config/redis");
const logger_1 = require("../config/logger");
const crypto_1 = require("crypto");
const RAPID_CLICK_THRESHOLD_MS = 30000; // 30 seconds
const IP_FLOOD_THRESHOLD = 10; // Max clicks per hour
const IP_FLOOD_WINDOW_MS = 3600000; // 1 hour
const RECORD_TTL_SECONDS = 3600; // 1 hour TTL on keys
const CLEANUP_INTERVAL_MS = 600000; // Clean every 10 minutes
const BOT_PATTERNS = [
    /googlebot/i,
    /bingbot/i,
    /slurp/i,
...
class ClickFraudService {
     ...
     */
    async detectFraudulentClick(campaignId, userId, ip, userAgent) {
     ...
     */
    async recordClick(campaignId, userId, ip, userAgent) {
        if (!campaignId || !userId) {
            logger_1.logger.warn('[ClickFraudService] Missing campaignId or userId');
            return;
        }
        const now = Date.now();
        ...
        const clickId = `${now}:${(0, crypto_1.randomUUID)().replace(/-/g, '').slice(0, 8)}`;
        ...
        try {
            ...
            logger_1.logger.warn('[ClickFraudService] Redis recordClick failed', { campaignId, userId, error: err });
    ...
    async getClickStats(campaignId) {
        const redis = (0, redis_1.getRedis)();
        try {
            // Count total user keys matching pattern (approximate)
            ...
            logger_1.logger.warn('[ClickFraudService] Redis getClickStats failed', { campaignId, error: err });
     ...
     */
    async checkRapidClick(campaignId, userId) {
        const redis = (0, redis_1.getRedis)();
        const userKey = `ads:fraud:user:${campaignId}:${userId}`;
        try {
            const now = Date.now();
            ...
            logger_1.logger.warn('[ClickFraudService] Redis checkRapidClick failed', { campaignId, userId, error: err });
        }
        return null;
    }
    /**
     ...
     */
    async checkIPFlooding(campaignId, ip) {
        if (!ip)
            return null;
        const redis = (0, redis_1.getRedis)();
        const ipKey = `ads:fraud:ip:${campaignId}:${ip}`;
        try {
            ...
            logger_1.logger.warn('[ClickFraudService] Redis checkIPFlooding failed', { campaignId, ip, error: err });
        }
        return null;
    }
    /**
    ...
    async cleanup() {
        ...
        try {
            ...
            if (cleaned > 0) {
                logger_1.logger.debug('[ClickFraudService] Cleanup completed', { cleanedEntries: cleaned });
            }
        }
        catch (err) {
            logger_1.logger.warn('[ClickFraudService] Redis cleanup failed', { error: err });
        }
    }
}
exports.clickFraudService = new ClickFraudService();
//# sourceMappingURL=clickFraudService.js.map