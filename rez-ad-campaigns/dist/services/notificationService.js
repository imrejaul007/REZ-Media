...
exports.notifyAdApproved = notifyAdApproved;
exports.notifyAdRejected = notifyAdRejected;
exports.notifySpendMilestone = notifySpendMilestone;
exports.notifyBudgetAlert = notifyBudgetAlert;
exports.notifyEngagementSpike = notifyEngagementSpike;
exports.notifyAdViewedNoClick = notifyAdViewedNoClick;
exports.notifyClickedNoConvert = notifyClickedNoConvert;
exports.checkSpendMilestones = checkSpendMilestones;
...
function getNotificationQueue() {
    if (!_queue) {
        _queue = new bullmq_1.Queue(NOTIFICATION_QUEUE_NAME, {
 ...
 */
async function notifyAdApproved(merchantId, adId, adTitle, placement) {
    try {
        ...
        logger_1.logger.info('[NotificationService] Ad approved notification sent', {
            merchantId,
            adId,
            eventType: 'ad_approved',
        });
        ...
        logger_1.logger.error('[NotificationService] Failed to send ad approved notification', {
            merchantId,
            adId,
 ...
 */
async function notifyAdRejected(merchantId, adId, adTitle, rejectionReason) {
    try {
        const queue = getNotificationQueue();
        const event = {
        ...
        logger_1.logger.info('[NotificationService] Ad rejected notification sent', {
            merchantId,
            adId,
            eventType: 'ad_rejected',
        });
        ...
        logger_1.logger.error('[NotificationService] Failed to send ad rejected notification', {
            merchantId,
            adId,
            error: error instanceof Error ? error.message : String(error),
        });
 ...
 */
async function notifySpendMilestone(merchantId, adId, adTitle, milestone, totalBudget, totalSpent) {
    try {
        const queue = getNotificationQueue();
        const event = {
        ...
        logger_1.logger.info('[NotificationService] Spend milestone notification sent', {
            merchantId,
            adId,
            milestone,
            totalSpent,
        ...
        logger_1.logger.error('[NotificationService] Failed to send spend milestone notification', {
            merchantId,
            adId,
            error: error instanceof Error ? error.message : String(error),
        });
 ...
 */
async function notifyBudgetAlert(merchantId, adId, adTitle, dailyBudget, dailySpent, alertType) {
    try {
        const queue = getNotificationQueue();
        ...
        logger_1.logger.info('[NotificationService] Budget alert notification sent', {
            merchantId,
            adId,
            alertType,
            dailyBudget,
        ...
        logger_1.logger.error('[NotificationService] Failed to send budget alert notification', {
            merchantId,
            adId,
            error: error instanceof Error ? error.message : String(error),
        });
 ...
 */
async function notifyEngagementSpike(merchantId, adId, adTitle, spikeType, currentValue, averageValue, percentIncrease) {
    try {
        const queue = getNotificationQueue();
        const event = {
        ...
        logger_1.logger.info('[NotificationService] Engagement spike notification sent', {
            merchantId,
            adId,
            spikeType,
            percentIncrease,
        ...
        logger_1.logger.error('[NotificationService] Failed to send engagement spike notification', {
            merchantId,
            adId,
            error: error instanceof Error ? error.message : String(error),
        });
 ...
 */
async function notifyAdViewedNoClick(userId, adId, adTitle, merchantName, ctaText, scheduledFor) {
    try {
        const queue = getNotificationQueue();
        const event = {
        ...
        logger_1.logger.info('[NotificationService] Re-target view notification scheduled', {
            userId,
            adId,
        ...
        logger_1.logger.error('[NotificationService] Failed to schedule re-target view notification', {
            userId,
            adId,
            error: error instanceof Error ? error.message : String(error),
        });
 ...
 */
async function notifyClickedNoConvert(userId, adId, adTitle, merchantName, ctaText, scheduledFor) {
    try {
        const queue = getNotificationQueue();
        const event = {
        ...
        logger_1.logger.info('[NotificationService] Follow-up click notification scheduled', {
            userId,
            adId,
            delayHours: Math.round(delayMs / 3600000),
        });
        ...
        logger_1.logger.error('[NotificationService] Failed to schedule follow-up click notification', {
            userId,
            adId,
            error: error instanceof Error ? error.message : String(error),
        });
 ...
 */
async function checkSpendMilestones(merchantId, adId, adTitle, totalBudget, totalSpent) {
 ...
 */
async function checkBudgetAlerts(merchantId, adId, adTitle, dailyBudget, totalBudget, dailySpent, totalSpent) {
    const redis = (0, redis_1.getRedis)();
    // Daily budget alerts
    if (dailyBudget > 0) {
