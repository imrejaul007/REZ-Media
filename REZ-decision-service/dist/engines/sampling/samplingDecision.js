...
exports.TimingEngine = exports.CoinAllocationEngine = exports.SamplingScoringEngine = void 0;
exports.makeSamplingDecision = makeSamplingDecision;
...
// SCORING ENGINE
// ============================================
class SamplingScoringEngine {
    /**
     * Calculate user's eligibility and score for a campaign
     */
    async score(context, config) {
        const reasons = [];
        let score = 0;
        let eligible = true;
        // 1. Fatigue check
     ...
     */
    async checkFatigue(userId) {
        const key = (suffix) => `${REDIS_PREFIX}fatigue:${userId}:${suffix}`;
     ...
     */
    async getUserAffinity(userId, merchantId) {
        if (!merchantId)
            return 0.5; // Neutral
...
class CoinAllocationEngine {
     ...
     * Determine coin type
     */
    determineCoinType(config, context) {
...
exports.CoinAllocationEngine = CoinAllocationEngine;
// ============================================
// TIMING ENGINE
// ============================================
class TimingEngine {
    ...
    getNextBestTime(currentHour) {
        // Next meal time in minutes
        if (currentHour < 12)
            return (12 - currentHour) * 60;
        if (currentHour < 19)
            return (19 - currentHour) * 60;
        return (24 - currentHour + 12) * 60;
    }
    getBestTimeLabel(currentHour) {
        if (currentHour < 12)
...
// ============================================
async function makeSamplingDecision(context, config) {
...
//# sourceMappingURL=samplingDecision.js.map