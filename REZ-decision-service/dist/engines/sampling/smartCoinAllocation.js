...
exports.SmartCoinAllocator = exports.MarketAnalyzer = exports.UserAnalyzer = exports.MerchantAnalyzer = exports.BudgetManager = void 0;
exports.allocateCoins = allocateCoins;
exports.getCoinAllocationBreakdown = getCoinAllocationBreakdown;
exports.getUserCoinStats = getUserCoinStats;
...
class BudgetManager {
     ...
     */
    async checkBudget(campaignId, userId, requestedAmount) {
     ...
     */
    async reserveBudget(campaignId, userId, amount) {
        const today = new Date().toISOString().split('T')[0];
        const weekStart = this.getWeekStart();
        const campaignDailyKey = `${BUDGET_PREFIX}campaign:${campaignId}:daily:${today}`;
        const userDailyKey = `${BUDGET_PREFIX}user:${userId}:daily:${today}`;
        ...
        try {
            ...
            console.error('[BudgetManager] Failed to reserve budget:', error);
            return false;
        }
    }
    /**
     ...
     */
    async releaseBudget(campaignId, userId, amount) {
        const today = new Date().toISOString().split('T')[0];
        const weekStart = this.getWeekStart();
        const pipeline = redis.pipeline();
        ...
        try {
            ...
            console.error('[BudgetManager] Failed to release budget:', error);
            return false;
        }
    }
    /**
     ...
     */
    async getCampaignBudgetStatus(campaignId) {
        const today = new Date().toISOString().split('T')[0];
        const dailySpent = parseInt(await redis.get(`${BUDGET_PREFIX}campaign:${campaignId}:daily:${today}`) || '0');
        return {
            dailySpent,
    ...
    async pauseCampaign(campaignId, reason) {
        ...
        console.log(`[BudgetManager] Campaign ${campaignId} paused: ${reason}`);
    }
    /**
     * Check if campaign is paused
     */
    async isCampaignPaused(campaignId) {
...
class MerchantAnalyzer {
    ...
    async getMerchantData(merchantId) {
        ...
        const [inventoryLevel, conversionRate, rating, totalRedeems] = await Promise.all([
     ...
     */
    async getAvgDailyRedeems(merchantId) {
     ...
     * Cache merchant data
     */
    async cacheMerchantData(merchantId, data) {
        const prefix = `${REDIS_PREFIX}merchant:${merchantId}`;
...
class UserAnalyzer {
     ...
     */
    async getUserData(userId, merchantId) {
        const prefix = `${REDIS_PREFIX}user:${userId}`;
        const [lastActivity, lifetimeCoins, categoryHistory] = await Promise.all([
     ...
     */
    async calculatePastConversionRate(userId) {
        const offered = parseInt(await this.redis.get(`${REDIS_PREFIX}user:${userId}:coinsOffered`) || '0');
        const redeemed = parseInt(await this.redis.get(`${REDIS_PREFIX}user:${userId}:coinsRedeemed`) || '0');
        if (offered === 0)
     ...
     */
    async recordAllocation(userId, merchantId, amount, outcome) {
        ...
        const merchantCategory = await this.redis.get(`${REDIS_PREFIX}merchant:${merchantId}:category`);
        if (merchantCategory) {
            await this.redis.hincrbyfloat(`${key}:categoryHistory`, merchantCategory, amount / 10);
        }
        // Record recent scan
...
exports.UserAnalyzer = UserAnalyzer;
// ============================================
// MARKET ANALYZER
// ============================================
class MarketAnalyzer {
     ...
     */
    async findNearbyCompetitors(merchantId, location) {
        if (!location)
            return 0;
        const merchantCategory = await this.redis.get(`${REDIS_PREFIX}merchant:${merchantId}:category`);
        if (!merchantCategory)
    ...
    isPeakHour(hour) {
        return ((hour >= DEFAULT_CONFIG.lunchStart && hour <= DEFAULT_CONFIG.lunchEnd) ||
            (hour >= DEFAULT_CONFIG.dinnerStart && hour <= DEFAULT_CONFIG.dinnerEnd));
    }
    isMealTime(hour) {
        return ((hour >= 12 && hour <= 13) || // Lunch peak
...
class SmartCoinAllocator {
    ...
    async allocate(request) {
        ...
        if (pauseCheck.paused) {
            return this.createExhaustedResponse(`Campaign paused: ${pauseCheck.reason}`);
        }
        ...
        if (location) {
            marketConditions.nearbyCompetitors = await this.marketAnalyzer.findNearbyCompetitors(merchantId, location);
        }
        ...
        // 11. Generate reason
        const reason = this.generateReason(userData, merchantData, marketConditions, budgetCheck.status);
        return {
            coins,
            breakdown,
            reason,
     ...
     */
    async recalculate(request, originalCoins) {
        ...
        if (newAllocation.coins !== originalCoins) {
            await this.budgetManager.releaseBudget(request.campaignId, request.userId, originalCoins);
            await this.budgetManager.reserveBudget(request.campaignId, request.userId, newAllocation.coins);
        }
    ...
    async validate(request) {
        ...
        // Check budget
        const budgetCheck = await this.budgetManager.checkBudget(request.campaignId, request.userId, request.baseCoins);
        if (!budgetCheck.allowed) {
    ...
    createExhaustedResponse(reason) {
        return {
            coins: 0,
            breakdown: {
                base: 0,
    ...
    generateReason(user, merchant, market, budgetStatus) {
        const parts = [];
        // User factors
        if (user.stage === 'NEW') {
...
exports.SmartCoinAllocator = SmartCoinAllocator;
// ============================================
// CONVENIENCE FUNCTIONS
// ============================================
async function allocateCoins(request) {
    const allocator = new SmartCoinAllocator();
    return allocator.allocate(request);
}
async function getCoinAllocationBreakdown(request) {
    ...
    if (request.location) {
        marketConditions.nearbyCompetitors = await marketAnalyzer.findNearbyCompetitors(request.merchantId, request.location);
    }
...
async function getUserCoinStats(userId) {
    const userAnalyzer = new UserAnalyzer();
...
//# sourceMappingURL=smartCoinAllocation.js.map