...
exports.campaignOptimizer = exports.CampaignOptimizer = void 0;
...
class CampaignOptimizer {
     ...
     */
    async recordRedemption(campaignId, coinsAwarded) {
        const key = `campaign:metrics:${campaignId}`;
     ...
     */
    async recordPurchase(campaignId, amount) {
        const key = `campaign:metrics:${campaignId}`;
     ...
     */
    async recordABTestResult(campaignId, variant, coinsAwarded) {
     ...
     */
    async recordABTestConversion(campaignId, variant) {
        const key = `campaign:abtest:${campaignId}:${variant}`;
     ...
     */
    async getOptimization(campaignId) {
        ...
        const { predictedOptimalCoins, predictedTargeting, historicalConfidence } = this.learnFromHistory(historicalData, metrics.channel);
        // Determine coin recommendation
        const recommendedCoins = this.calculateRecommendedCoins(conversionRate, metrics.coinsSpent, metrics.redemptions, predictedOptimalCoins, abTestResults);
        // Determine targeting recommendation
        const recommendedTargeting = this.calculateRecommendedTargeting(metrics.targeting, predictedTargeting, conversionRate);
        // Calculate estimated conversion rate using weighted average
        const estimatedConversionRate = this.weightedAverageConversionRate(conversionRate, historicalData.length > 0
            ? historicalData.reduce((sum, h) => sum + h.conversionRate, 0) / historicalData.length
            : 0, historicalConfidence);
        ...
        // Generate reason
        const reason = this.generateOptimizationReason(conversionRate, recommendedCoins, recommendedTargeting, abTestResults);
        return {
            recommendedCoins,
     ...
     */
    calculateRecommendedCoins(currentConversionRate, currentCoinsSpent, redemptions, predictedOptimal, abTestResults) {
     ...
     */
    calculateRecommendedTargeting(currentTargeting, predictedTargeting, conversionRate) {
     ...
     */
    async storeCampaignData(config) {
     ...
     */
    learnFromHistory(historicalData, channel) {
        ...
        const targetingPerformance = new Map();
     ...
     */
    async predictOptimalCoins(targeting, channel) {
        const historicalData = await this.getHistoricalData();
        const relevant = historicalData.filter(h => h.targeting.some(t => targeting.includes(t)) && h.channel === channel);
        if (relevant.length < 5) {
            // Fall back to segment-based prediction
            const avgSegmentScore = targeting.reduce((sum, t) => sum + this.getSegmentPerformance(t), 0) / targeting.length;
            return {
     ...
     */
    async checkAlerts(campaignId, config) {
        const alerts = [];
        const metrics = await this.getMetrics(campaignId);
        ...
        if (alerts.length > 0) {
            await Promise.all(alerts.map(alert => this.redis.lpush(`campaign:alerts:${campaignId}`, JSON.stringify(alert))));
        }
        return alerts;
    }
    /**
     ...
     */
    async detectAnomalyForChannel(campaignId) {
        const historicalData = await this.getHistoricalData();
    ...
    async detectAnomalies(campaignId, metrics) {
    ...
    async detectConversionRateAnomaly(campaignId, metrics) {
        const historicalData = await this.getHistoricalData();
        if (historicalData.length < 10)
            return null;
    ...
    async detectActivityAnomaly(campaignId, metrics) {
     ...
     */
    async getAlerts(campaignId, limit = 10) {
    ...
    async optimizeBudgetAllocation(campaigns) {
        ...
        for (const [channel, data] of channelMetrics.entries()) {
            ...
            const avgConversionRate = data.metrics.reduce((sum, m) => sum + (m.scans > 0 ? m.redemptions / m.scans : 0), 0) / data.metrics.length;
     ...
     */
    weightedAverageConversionRate(current, historical, historicalWeight) {
     ...
     */
    generateOptimizationReason(conversionRate, recommendedCoins, recommendedTargeting, abTestResults) {
        const reasons = [];
...
// EXPORTS
// ============================================
exports.campaignOptimizer = new CampaignOptimizer();
exports.default = exports.campaignOptimizer;
//# sourceMappingURL=campaignOptimizer.js.map