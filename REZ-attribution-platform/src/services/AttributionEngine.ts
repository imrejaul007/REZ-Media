...
import {
  ...
  ITouchpointContribution
} from '../models/AttributionReport';
import logger from '../utils/logger';

// Type for touchpoint data (works with both Document and lean results)
type TouchpointData = {
  id: string;
  userId: string;
  sessionId: string;
...
export class AttributionEngine {
  private readonly defaultConfig: AttributionConfig = {
  ...
  calculateConversionAttribution(
    ...
    const eligibleTouchpoints = this.filterTouchpointsByWindow(
      touchpoints,
      conversion.conversionTimestamp,
      fullConfig.attributionWindow
    );
    ...
    const contributions = this.applyAttributionModel(
      sortedTouchpoints,
      fullConfig.model,
      conversion.value || 1
    );
   ...
   */
  private sortByTimestamp(touchpoints: TouchpointData[]): TouchpointData[] {
  ...
  async generateAttributionReport(
    ...
    const allContributions: Map<string, AttributionResult[]> = new Map();
    const channelTotals: Map<string, IChannelAttribution> = new Map();
    const campaignTotals: Map<string, ICampaignAttribution> = new Map();
    ...
    for (const conversion of conversions) {
      ...
      for (const contribution of attribution.contributions) {
        ...
        if (contribution.attributedCampaignId) {
          ...
          const channelInCampaign = currentCampaign.channelBreakdown.find(
            c => c.channel === channel
          );
    ...
    for (const [channel, data] of channelTotals) {
      data.conversions = conversions.filter(c => {
        ...
        return contributions.some(
          contrib => contrib.attributedChannel === channel && contrib.contribution > 0
        );
      }).length;
    }
    ...
    for (const [campaignId, data] of campaignTotals) {
      data.conversions = conversions.filter(c => {
        const contributions = allContributions.get(c.id) || [];
        return contributions.some(
          contrib => contrib.attributedCampaignId === campaignId && contrib.contribution > 0
        );
      }).length;
    }
    ...
    const touchpointContributions: ITouchpointContribution[] = [];
  ...
  private generateFunnelData(
    touchpoints: TouchpointData[],
    conversions: IConversion[]
  ): Array<{ stage: string; count: number; dropoffRate: number }> {
  ...
  async compareAttributionModels(
    touchpoints: TouchpointData[],
    conversion: IConversion
  ): Promise<Record<AttributionModel, ConversionAttribution>> {
    const models = Object.values(AttributionModel);
    const results: Record<string, ConversionAttribution> = {};

    for (const model of models) {
      results[model] = this.calculateConversionAttribution(
        touchpoints,
        conversion,
        { model }
      );
    ...
    return results as Record<AttributionModel, ConversionAttribution>;
  }
}
